...
namespace margelo
{
	void install(Runtime& jsiRuntime)
	{
		...
		auto tplHelloWorld = Function::createFromHostFunction(
			jsiRuntime, PropNameID::forAscii(jsiRuntime, "generateHelloWorld"), 1,
			[](Runtime &runtime, const Value &thisValue,const Value *arguments,
			   size_t count) -> Value
			{
				// int length = arguments[0].getNumber();
        ...
        jsiRuntime.global().setProperty(jsiRuntime, "tplHelloWorld", std::move(tplHelloWorld));


		auto helloFibo = Function::createFromHostFunction(
			jsiRuntime, PropNameID::forAscii(jsiRuntime, "generateFibo"), 1,
			[](Runtime &runtime, const Value &thisValue,const Value *arguments,
			   size_t count) -> Value
			{
				// int number1 = arguments[0].getNumber();
				// std::string secret = helloWorld();
				int index = static_cast<int>(arguments[0].getNumber());
				Array resultArray(runtime,index);
				for(int i=0; i<index; ++i){
					unsigned long long fibNumber = fibonacci(i);
					resultArray.setValueAtIndex(runtime,i,Value(static_cast<double>(fibNumber)));
				}
				
				return resultArray;
			});

        jsiRuntime.global().setProperty(jsiRuntime, "helloFibo", std::move(helloFibo));
	...
	{
		std::string testVar = "test string";
